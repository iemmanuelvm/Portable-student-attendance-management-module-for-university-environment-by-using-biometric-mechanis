...
def generarReporteAlumno(Matricula,NombreAlumno,Docente,Curso,CicloEscolar,TotalAsistencias,ProgramaAcademico,Retardos,ClasesTotales,RPA,HISTORIAL):
	...
	Story = []
	logotipo = os.getcwd()+"/icons/uaz2.png"
	...
	if RPA != 0:
		...
		estilos = getSampleStyleSheet()
		estilos.add(ParagraphStyle(name='Asistencia', alignment = TA_JUSTIFY, fontSize = 12, borderColor = 'black', fontName = "Times-Roman", spaceBefore = 12))
		texto = 'Cada '+str(RPA)+' retardos equivalen a 1 Falta, por lo tanto, el alumno obtuvo un total de retardos ' +str(Retardos)+' que equivalen a ' + str(count) +' faltas'
		Story.append(Paragraph(texto, estilos["Asistencia"]))
		estilos = getSampleStyleSheet()
		estilos.add(ParagraphStyle(name='Asistencia', alignment = TA_JUSTIFY, fontSize = 12, borderColor = 'black', fontName = "Times-Roman", spaceBefore = 12))
